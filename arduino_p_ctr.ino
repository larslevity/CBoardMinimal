...
// PSens
float output_pressure_sensor, output_min = 1638, output_max = 14745; float pressure_max = 10.3421, pressure_min = 0; byte byte_one, byte_two; double currentPressure; // Reference int pwm_value; float DC; double referencePressure;
...
void loop() {
   ...
   // Read PSens
   Wire.beginTransmission(pressureSensor); //Begin Transmission to Pressure Sensor
   Wire.requestFrom(pressureSensor, 2);   //Request two bytes from  
Pressure Sensor for Pressure reading
   byte_one=Wire.read();                     // Save first read Byte
   byte_two=Wire.read();                     // Save second read Byte
   output_pressure_sensor=byte_one*256+byte_two;             //  
calculate output using byte_one and two
    
currentPressure=(((output_pressure_sensor-output_min)*(pressure_max-pressure_min))/(output_max-output_min))+pressure_min; //Transfer function to get pressure in [Bar] from calculated output

   u_ctr = ctr_out(referencePressure, currentPressure);
   u = mapping(u_ctr);
   analogWrite(PWM_PIN_OUT, u);
   delay(tsampling-4);                   // -4 : give calculations  
some time ...
